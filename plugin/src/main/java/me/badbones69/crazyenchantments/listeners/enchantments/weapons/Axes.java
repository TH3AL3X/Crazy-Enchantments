package me.badbones69.crazyenchantments.listeners.enchantments.weapons;
    ...
    public void onPlayerDeath(PlayerDeathEvent e) {
        ...
        if (e.getEntity().getKiller() != null) {
            ...
            if (ce.hasEnchantment(item, CEnchantments.DECAPITATION) && CEnchantments.DECAPITATION.chanceSuccessful(item)) {
                ...
                Bukkit.getPluginManager().callEvent(event);
                if (!event.isCancelled()) {
                    e.getDrops().add(new ItemBuilder().setMaterial("PLAYER_HEAD").setPlayer(player.getName()).build());
                }
            }
        }
    }
